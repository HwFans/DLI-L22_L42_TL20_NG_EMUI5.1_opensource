...
static ssize_t lcdkit_check_esd(void* pdata)
{
    ...
    for(j = 0;j < lcdkit_info.panel_infos.esd_cmds.cmd_cnt;j++)
    {
        count = 0;
        do
        {
            compare_reg = 0;
if(strstr(lcdkit_info.panel_infos.panel_name, "HX8394F") != NULL)
{
            mdss_dsi_panel_cmd_read(pdata,lcdkit_info.panel_infos.esd_cmds.cmds[j].payload[0],0x00,NULL,read_value,3);
}
else
{
            mdss_dsi_panel_cmd_read(pdata,lcdkit_info.panel_infos.esd_cmds.cmds[j].payload[0],0x00,NULL,read_value,lcdkit_info.panel_infos.esd_cmds.cmds[j].dlen - 1);
}
            count++;
            for (i = 0; i < lcdkit_info.panel_infos.esd_cmds.cmds[j].dlen - 1; i++)
            {
                if (read_value[i] != lcdkit_info.panel_infos.esd_cmds.cmds[j].payload[i+1])
                {
                    compare_reg = -1;
                    LCDKIT_ERR("panel esd check error: in %s,  reg 0x%02X %d byte should be 0x%02x, but read data =0x%02x \n",
                       __func__,lcdkit_info.panel_infos.esd_cmds.cmds[j].payload[0],i+1,lcdkit_info.panel_infos.esd_cmds.cmds[j].payload[i+1],read_value[i]);
                    ret = 1;
                    break;
                }
            }
