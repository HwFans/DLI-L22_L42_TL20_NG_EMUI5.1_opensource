...
void novatek_kit_parse_specific_dts(struct ts_kit_device_data *chip_data)
{
	...
	if (nvt_ts->criteria_threshold_flag) {
		...
		retval = of_property_read_u32_array(device, "PS_Config_Lmt_Open_Rawdata_P_A", nvt_ts->PS_Config_Lmt_Open_Rawdata_P_A, (IC_X_CFG_SIZE * IC_Y_CFG_SIZE));
		if (retval) {
		...
		retval = of_property_read_u32_array(device, "PS_Config_Lmt_Open_Rawdata_N_A", nvt_ts->PS_Config_Lmt_Open_Rawdata_N_A, (IC_X_CFG_SIZE * IC_Y_CFG_SIZE));
		if (retval) {
...
static int novatek_get_info(struct ts_chip_info_param *info)
{
	...
	|| (atomic_read(&nvt_ts->chip_data->ts_platform_data->state) == TS_WORK_IN_SLEEP)) {
		...
		if(strstr(novatek_kit_project_id, "VCTO33100")) {
			...
			snprintf(info->mod_vendor, PAGE_SIZE, "%s",nvt_ts->chip_data->module_name);
		}
